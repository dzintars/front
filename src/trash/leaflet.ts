...
import { LeafletModuleType, setupLeafletMap } from '../../common/dom/setup-leaflet-map'
...
class HaMap extends LitElement {
  ...
  private async loadMap(): Promise<void> {
    ;[this._leafletMap, this.Leaflet] = await setupLeafletMap(this._mapEl, this.darkMode)
    this._drawEntities()
    this._leafletMap.invalidateSize()
    this._fitMap()
  }
  ...
  private _fitMap(): void {
    ...
    if (this._mapItems.length === 0) {
      this._leafletMap.setView(
        new this.Leaflet.LatLng(this.hass.config.latitude, this.hass.config.longitude),
        this.zoom || 14
      )
      return
    }
